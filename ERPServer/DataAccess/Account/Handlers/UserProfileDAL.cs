...
namespace Account.DataAccessLayer
{
    public class UserProfileDAL : IUserProfileDAL
    {
        ...
        {
            return await _appDbContext.UserProfiles.Include(x=>x.Company).ThenInclude(x=>x.Setting).Include(x=>x.Role).ThenInclude(x=>x.RolePrivileges).SingleOrDefaultAsync(x => x.AppUserId == appUserId);
        }
